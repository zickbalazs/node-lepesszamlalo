    ...
    if (req.session.loggedin){
        connection.query('select * from stepdatas where uid=?', [req.session.loggedid], (err,data)=>{
            if (err) res.status(500).send(err.sqlMessage);
                ...
                ejs.renderFile('./views/pages/chart.ejs', ({data:cfg.config, user:req.session, records:data}), (err,data)=>{
                    if (err) res.status(500).send(err.message);
    ...
    if (req.session.loggedin){
        connection.query('select * from stepdatas where uid=?', [req.session.loggedid], (err,data)=>{
            if (err) res.status(500).send(err.sqlMessage);
                ...
                ejs.renderFile('./views/pages/calendar.ejs', ({data:cfg.config, user:req.session, records:data}), (err,data)=>{
                    if (err) res.status(500).send(err.message);
                    res.status(200).send(data);
                })
            }
